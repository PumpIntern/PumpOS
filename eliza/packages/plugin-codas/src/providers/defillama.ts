...
async function fetchTVLData(): Promise<TVLMetrics> {
  ...
  try {
    ...
    const confluxData = chainResponse.data.find((chain) => chain.name.toLowerCase() === 'conflux');
    const confluxTvl = confluxData?.tvl || 0;
    const confluxPercentage = confluxTvl / totalTvl;
    ...
    const confluxProtocols = protocolsResponse.data
      .filter((protocol) => protocol.chain === 'Conflux' || (protocol.chainTvls && protocol.chainTvls.Conflux > 0))
      .map((protocol) => ({
        name: protocol.name,
        tvl: protocol.chainTvls?.Conflux || (protocol.chain === 'Conflux' ? protocol.tvl : 0)
      }))
      .sort((a, b) => b.tvl - a.tvl)
      .slice(0, 3);
  ...
  async get(_runtime: IAgentRuntime, _message: Memory, _state?: State): Promise<string> {
    try {
      ...
      return [
        ...
        `• Market Share: ${formatPercentage(data.globalMetrics.confluxPercentage)}`,
        '',
        'Top Conflux Protocols:',
